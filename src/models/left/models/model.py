...
from models.left.domain import (get_description_caegories_v2, read_concepts_v2,
                                read_description_categories)
from models.left.generalized_fol_parser import NCGeneralizedFOLPythonParser
from models.left.models.concept.concept_embedding import (
    NCLinearConceptEmbedding, NCVSEConceptEmbedding)
...
class LeftModel(nn.Module):
    ...
    def __init__(self, domain, output_vocab: Optional[Vocab] = None, fmri_args: Dict[str, Any] = None):
        super().__init__()
        self._def_configs()
        ...
        if configs.model.scene_graph == '2d':
            import models.left.nn.scene_graph.scene_graph_2d as sng
            ...
            import models.left.nn.scene_graph.scene_graph_3d as sng
            ...
            import models.left.nn.scene_graph.scene_graph_skeleton as sng
        ...
        if configs.model.concept_embedding == 'vse':
            ...
            from models.left.models.reasoning.reasoning import LeftGrounding
            ...
            from models.left.models.reasoning.reasoning import \
                NCOneTimeComputingGrounding
        ...
        from models.left.generalized_fol_executor import \
        ...
        self.executor = NCGeneralizedFOLExecutor(self.domain, self.parser, fmri_args=fmri_args)

        from models.left.models.losses import (AttrClsLoss, PickPlaceLoss,
                                               QALoss, RefExpLoss)
    ...
    def extract_description_categories_v2(self, domain: FunctionDomain) -> List[str]:
        return get_description_caegories_v2(domain)
    ...
    def execute_program_from_queries(self, raw_parsing: str, grounding, outputs: Dict[str, Any]):
        parsing, program, execution, trace = None, None, None, None
        with self.executor.with_grounding(grounding):
            try:
                try:
                    ...
                    raise ExecutionFailed('Parsing failed for question: {}.'.format(raw_parsing)) from e
                ...
                try:
                    if not self.training:
                        with self.executor.record_execution_trace() as trace_getter:
                            ...
                            self.executor._sub_execution_trace = list()
                    else:
                        execution = self.executor.execute(program)
                        # @Yanchen Wang: Add sub_execution_trace
                        self.executor._sub_execution_trace = list()
                    ...
                    logger.exception('Execution failed for question: {}\nProgram: {}.'.format(raw_parsing, program))
                    raise ExecutionFailed('Execution failed for question: {}\nProgram: {}.'.format(raw_parsing, program)) from e
            except ExecutionFailed as e:
                print(e)
