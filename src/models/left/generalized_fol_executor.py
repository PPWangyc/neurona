...
from concepts.dsl.executors.function_domain_executor import \
...
class NCGeneralizedFOLExecutor(FunctionDomainExecutor):
    def __init__(self, domain: FunctionDomain, parser: Optional[ParserBase] = None, allow_shift_grounding=False, fmri_args: Dict[str, Any] = None):
        ...
        # @Yanchen Wang: Add sub_execution_trace
        self._sub_execution_trace = list()
    ...
    def _execute(self, expr: E.Expression) -> TensorValue:
        ...
        # @Yanchen Wang: Add sub_execution_trace
        self._sub_execution_trace.append((expr, rv))
    ...
    def _execute_inner(self, expr: E.Expression) -> TensorValue:
        if isinstance(expr, E.BoolExpression):
            ...
            if expr.bool_op is E.BoolOpType.AND:
                ...
                # print(f'expanded_tensors: {expanded_tensors}')
            ...
            if expr.function.name in self.function_implementations:
                ...
                if len(args) == 1:
                    ...
                    # grounding attribute concept to brain relation
                    if expr.function.name.endswith('_Object') and self.fmri_args['ground_attr_relation']:
                        rel_func_name = expr.function.name[:-7] + '_Object_Object'
                        rel_grounding_tensor = self.grounding.compute_similarity('relation', rel_func_name)
                        rel_grounding_tensor = _do_apply_self_mask(rel_grounding_tensor)
                        # get the max of rel_grounding_tensor in dim 0
                        # @Yanchen Wang: Get only the values, not the indices
                        # TODO: Need to get the indices of the max values and use them to index into grounding_tensor
                        # if self.fmri_args['ground_rel_guided_relation'] != 'guide-off':
                        #     avg_grounding_tensor = grounding_tensor.mean(0, keepdim=True).squeeze()
                        #     rel_grounding_tensor = grounding_tensor @ rel_grounding_tensor
                        #     rel_grounding_tensor = rel_grounding_tensor + avg_grounding_tensor
                        # else:
                        #     rel_grounding_tensor = rel_grounding_tensor.mean(0, keepdim=True).squeeze()
                        rel_grounding_tensor = rel_grounding_tensor.mean(0, keepdim=True).squeeze()
                        # grounding_tensor = torch.max(rel_grounding_tensor, grounding_tensor)
                        grounding_tensor = rel_grounding_tensor + grounding_tensor
                        
                elif len(args) == 2:
                    # print(args)
                    subj_value = self._sub_execution_trace[1][1].tensor
                    obj_value = self._sub_execution_trace[4][1].tensor
                    if len(self.view_stack) > 0:
                        ...
                        # print(f"computed grounding tensor for relation: {expr.function.name} with shape {grounding_tensor.shape}")
                        # grounding_tensor = grounding_tensor.T # NS3D repro
                        if self.fmri_args['ground_rel_region']:
                            attr_name = expr.function.name[:-7]
                            region_grounding_tensor = self.grounding.compute_similarity('attribute', attr_name)
                else:
                    assert len(args) == 3
                    grounding_tensor = self.grounding.compute_similarity('multi_relation', expr.function.name)
                    raise NotImplementedError('This version of multi relation not implemented')
                # e.g., temporal shift
                    ...
                    if isinstance(arg, Variable):
                        ...
                        if not shift:
                            ...
                            if self.fmri_args['ground_rel_guided_relation'] == 'guide-all':
                                grounding_tensor = grounding_tensor.mean(0, keepdim=True).squeeze() + subj_value + obj_value
                            elif self.fmri_args['ground_rel_guided_relation'] == 'guide-object':
                                grounding_tensor = grounding_tensor.mean(0, keepdim=True).squeeze() + obj_value
                            elif self.fmri_args['ground_rel_guided_relation'] == 'guide-subject':
                                grounding_tensor = grounding_tensor.mean(0, keepdim=True).squeeze() + subj_value
                            elif self.fmri_args['ground_rel_guided_relation'] == 'guide-off':
                                grounding_tensor = grounding_tensor.mean(0, keepdim=True).squeeze()
                            if self.fmri_args['ground_rel_region']:
                                grounding_tensor = grounding_tensor + region_grounding_tensor
                            break
            ...
            if expr.quantification_op == 'iota':
                ...
                if expr.variable.dtype.typename == 'Object':
                    ...
                    # print(expr)
                    value = self._execute(expr.expression.arguments[1])
                    # subject value is after iota expression
                    # @Yanchen Wang: describe binary relation
                    subj_value = self._sub_execution_trace[2][1].tensor
                    obj_value = self._sub_execution_trace[5][1].tensor
                    # assert len(value.batch_variables) == 1, f'Variable {expr.variable.name} is not the only batch variable in {value.batch_variables}.'
                    # print(f"{expr.expression.arguments[1]} value shape: {value.tensor.shape} and compute attributedescription for {expr.variable.dtype.typename}")
                    answer = self.grounding.compute_description('attribute', expr.variable.dtype.typename, subj_value, obj_value)

                    # NB(Jiayuan Mao @ 2023/03/18): this does not support arbitrary arity!
                    # @Yanchen Wang: compute relation description score
                    # answer = value.tensor @ answer
                    # print(f"final answer shape: {answer.shape}")
                    # print(f"argmax of answer: {answer.argmax(dim=-1)}")
                    return TensorValue(expr.return_type, [], answer, quantized=False)
    ...
    def position(self, obj1, obj2):
        """Handle position function to determine if obj1 is on the left of obj2
        Returns:
            TensorValue: A tensor of indices (0 for left, 1 for right)
        """
        # Get object features
        obj1_features = obj1.tensor
        obj2_features = obj2.tensor
        
        # Get position prediction from scene graph
        position_indices = self.scene_graph.predict_position(obj1_features, obj2_features)
        
        # Create a new tensor with the result
        return TensorValue(INT64, [], position_indices.float(), quantized=False)
